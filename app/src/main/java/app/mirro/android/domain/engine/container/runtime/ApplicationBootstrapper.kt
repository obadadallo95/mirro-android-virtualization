    ...
    fun bootstrap(
        ...
        try {
            ...
            try {
                ...
                virtualContext.withHostIdentityForSystemCalls {
                ...
                // Android initializes manifest providers before Application.onCreate. Compose and
                // AndroidX startup rely on that ordering, so reproduce it inside the container.
                virtualContext.targetProviders += TargetFrameworkProviderBootstrapper().initialize(
                    virtualContext = virtualContext,
                    classLoader = classLoader,
                    log = log
                )
                ...
                log("Stage 4 failed: target framework provider initialization threw an exception")
                return buildFailureResult(stage, e, appClassLoaderInfo, virtualContextClassLoaderInfo, threadClassLoaderInfo, log)
            }
            stage = ApplicationBootstrapStage.BASE_CONTEXT_ATTACHED
