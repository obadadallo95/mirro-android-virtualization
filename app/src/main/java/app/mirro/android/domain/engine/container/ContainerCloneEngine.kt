 ...
 * redirection, and dedicated WebView data directory partitions without requiring Android-managed
 * profiles, secondary users, or device management.
 */
class ContainerCloneEngine(
    ...
    override suspend fun createClone(app: InstalledApp, config: CloneConfig): EngineExecutionResult<CloneInstance> {
        return try {
            ...
            virtualFileSystem.createSandbox(
                cloneId = cloneId,
    ...
    override suspend fun launchInstance(instance: CloneInstance): EngineExecutionResult<Unit> {
        return try {
            ...
            EngineExecutionResult.Pending(
                message = "Opening ${instance.customName}; runtime state will be updated after bootstrap."
            )
        } catch (e: Throwable) {
            EngineExecutionResult.Failure(
                error = e,
