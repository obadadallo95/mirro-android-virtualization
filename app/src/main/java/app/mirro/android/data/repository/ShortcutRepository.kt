...
class ShortcutRepository(
    ...
    fun requestPinShortcut(instance: CloneInstance): Boolean {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            // The callback is only a completion notification from the launcher. Passing the
            // clone launch intent here makes Samsung Launcher open the clone immediately after
            // pinning, which is surprising and makes the button look like "Add and launch".
            // The shortcut itself already owns the launch intent, so no callback is required.
